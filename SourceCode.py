...
def build_preset_buttons():
    for w in monaural_beats_frame.winfo_children():
        w.destroy()
    for w in binaural_beats_frame.winfo_children():
        w.destroy()

    allp = get_all_presets()

    for i, p in enumerate(allp["Monaural"]):
        btn = ttk.Button(
            monaural_beats_frame,
            text=p["label"],
            command=lambda l=p["left_hz"], r=p["right_hz"]: apply_preset(l, r)
        )
        btn.grid(row=i, column=0, padx=10, pady=10, sticky="ew")

    for i, p in enumerate(allp["Binaural"]):
        btn = ttk.Button(
